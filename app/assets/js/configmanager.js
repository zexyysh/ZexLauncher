...
const dataPath = path.join(sysRoot, '.zexcraft')
 ...
 * 
 * @param {string} displayName The desired in-game username.
 * 
 * @returns {Object} The authenticated account object created by this action.
 */
exports.addOfflineAuthAccount = function(displayName){
    ...
    const offlineUuid = crypto.createHash('md5').update('OfflinePlayer:' + displayName).digest('hex')
...
function _ensureJavaConfigEntry(serverid) {
    if(!Object.prototype.hasOwnProperty.call(config.javaConfig, serverid)) {
        config.javaConfig[serverid] = defaultJavaConfig17(null)
    }
}
...
exports.getMinRAM = function(serverid){
    _ensureJavaConfigEntry(serverid)
