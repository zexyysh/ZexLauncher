...
class ProcessBuilder {
    ...
    _lteMinorVersion(version) {
        if (!this.modManifest || !this.modManifest.id || !this.modManifest.id.includes('-')) {
            return false
        }
    ...
    constructJVMArguments(mods, tempNativePath){
        if (this.modManifest == null) {
            return this._constructJVMArgumentsVanilla(mods, tempNativePath)
        }
        if(mcVersionAtLeast('1.13', this.server.rawServer.minecraftVersion)){
    ...
    _constructJVMArgumentsVanilla(mods, tempNativePath){
        if(mcVersionAtLeast('1.13', this.server.rawServer.minecraftVersion)){
            return this._constructJVMArgumentsVanilla113(mods, tempNativePath)
        } else {
            return this._constructJVMArgumentsVanilla112(mods, tempNativePath)
        }
    }

    _constructJVMArgumentsVanilla113(mods, tempNativePath){
        const argDiscovery = /\${*(.*)}/
        let args = [...this.vanillaManifest.arguments.jvm]

        if(process.platform === 'darwin'){
            args.push('-Xdock:name=ZexLauncher')
        ...
        // Main Java Class
        args.push(this.vanillaManifest.mainClass)
                ...
                if(argDiscovery.test(args[i])){
                    ...
                    switch(identifier){
                        ...
                        case 'natives_directory':
                            ...
                            break
                        case 'launcher_name':
                            val = args[i].replace(argDiscovery, 'Zex-Launcher')
                            break
    ...
    _constructJVMArgumentsVanilla112(mods, tempNativePath){
        let args = []
        ...
        // Game Arguments
        let mcArgs = this.vanillaManifest.minecraftArguments.split(' ')
        const argDiscovery = /\${*(.*)}/
    ...
    classpathArg(mods, tempNativePath){
        ...
        if(!mcVersionAtLeast('1.17', this.server.rawServer.minecraftVersion) || this.usingFabricLoader || this.modManifest == null) {
