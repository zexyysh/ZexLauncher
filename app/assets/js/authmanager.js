...
/**
 * Add an offline (cracked) account. No authentication is required.
 * 
 * @param {string} username The desired in-game username.
 * @returns {Promise.<Object>} Promise which resolves the offline account object.
 */
    ...
    try {
        if(username == null || username.trim().length === 0) {
            return Promise.reject({
                ...
                desc: 'Lütfen geçerli bir kullanıcı adı girin.'
            })
        }
        ...
        if(!/^[a-zA-Z0-9_]{3,16}$/.test(username.trim())) {
            return Promise.reject({
                title: 'Geçersiz Kullanıcı Adı',
                desc: 'Kullanıcı adı 3-16 karakter uzunluğunda olmalı ve sadece harf, rakam ve alt çizgi içermelidir.'
            })
        }
        
        const ret = ConfigManager.addOfflineAuthAccount(username.trim())
        ConfigManager.save()
        return ret
    } catch(err) {
        log.error(err)
        return Promise.reject({
            title: 'Hata',
            desc: 'Offline hesap eklenirken bir hata oluştu.'
        })
    }
}
